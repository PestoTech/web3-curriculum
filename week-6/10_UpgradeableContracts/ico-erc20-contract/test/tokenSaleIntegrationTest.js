...
describe("TokenSale", function () {
    ...
    describe("Token Sale Transactions", function () {
        ...
        describe("Upgrading", function () {
            it("Upgrading contracts should retain state", async function () {
                ...
                const mpgv2 = await upgrades.upgradeProxy(mpg.address, MPGV2);
                
                const TokenSaleV2 = await ethers.getContractFactory("TokenSaleV2");
                const tokenSalev2 = await upgrades.upgradeProxy(tokenSale.address, TokenSaleV2);

                // Check the upgradbility of TokenSale contract
                // Check that the withdraw function is executed and it updates ether balances
                await expect(tokenSalev2.withdraw()).to.changeEtherBalances(
                    [tokenCreator, tokenSalev2],
                    [ONE_ETH, ethers.utils.parseEther("-1.0")]
                );

                // Check the upgradbility of MyPestoGems contract
                // Check that the burn function is executed and it updates token balances
                await expect(mpgv2.burn( ONE_ETH )).to.emit(mpgv2, "Transfer")
                    .withArgs(tokenCreator.address, ethers.constants.AddressZero, ONE_ETH);
            });
        });
    });
});
